...
try{
 ...
 if(! password) return res.send({status : false , message : "password is required "})
  ...
  if(!checkDetails) return res.send({status :false   , message : "please enter valid UserName And Password "})
...
exports.getUserData = async function (req, res) {
  ...
  let findData = await userModel.findById({_id : data})
...
let data = await userModel.findOneAndUpdate({_id:userId },{isDeleted : true} , {new : true})
