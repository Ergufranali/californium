...
const getBooksInyear = async function (req, res) {
  ...
  let particularsBooks = await BookModel.find(particularBooks);
  res.send({ msg: particularsBooks });
};

const getXINRBooks = async function (req, res) {
  let INRbooks = await BookModel.find({
    $or: [
      { "prices.indianPrice": "150rs" },
      { "prices,indianPrice": "200rs" },
      { "prices.indianPrice": "250rs" },
    ],
  });
...
const getRandomBooks = async function (req, res) {
  let randomBooks = await BookModel.find({
    $or: [{ totalPages: { $gte: 500 } }, { stockAvailable: true }],
  });
